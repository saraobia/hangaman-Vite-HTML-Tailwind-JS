//TO DO --> Pulire codice, inserire API per gioco normale, gestire Json esterno per categorie
...
let paroleMaggiorenni = ['zibellino'];
let contaErrori = 0;
...
let messaggio =
  eta >= 18
    ? (parolaCasuale =
        paroleMaggiorenni[Math.floor(Math.random() * paroleMaggiorenni.length)])
    : (parolaCasuale =
        paroleMinorenni[Math.floor(Math.random() * paroleMinorenni.length)]);
...
//Funzione per funzionamento del gioco
function hangman() {
